...
<?php
//echo 111;
//ppr($_SESSION["CITY_SELECT"]);

if(!isset($_SESSION["CITY_SELECT"]["NAME"]))
{
    $arUserGeoData = cmGetArUserGeoData();
    $_SESSION["CITY_SELECT"]["NAME"] = $arUserGeoData['cityCode'];
    $_COOKIE['CITY_SELECT']['NAME'] = $arUserGeoData['cityCode'];
//    header('Location: https://www.arbus.biz/?city='.$arUserGeoData['cityCode']);
}
...
if(!isset($_SESSION["CITY_SELECT"]["NAME"]))
{
    ...
    $SxGeo = new SxGeo($_SERVER["DOCUMENT_ROOT"].'/include/geo/SxGeoCity.dat', SXGEO_BATCH | SXGEO_MEMORY);
    mb_internal_encoding('8bit');
    $cityArray = $SxGeo->getCityFull($_SERVER["REMOTE_ADDR"]);
    $arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", "PROPERTY_REGION", "PROPERTY_PHONE");
    $arFilter = Array("IBLOCK_ID"=>1, "ACTIVE"=>"Y", "PROPERTY_REGION" => $cityArray["region"]["name_ru"]);
    $res = CIBlockElement::GetList(Array("sort"=>"asc"), $arFilter, false, Array("nPageSize"=>50), $arSelect);
    while($ob = $res->GetNextElement())
    {
        $arFields = $ob->GetFields();
        $_SESSION["CITY_SELECT"]["NAME"] = $arFields["NAME"];
        $_SESSION["CITY_SELECT"]["PHONE"] = $arFields["PROPERTY_PHONE_VALUE"];
    }
    unset($SxGeo);
}
...
if(isset($_GET["city"]))
{
    //echo "2133333333333";
    $arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", "PROPERTY_REGION", "PROPERTY_PHONE");
    $arFilter = Array("IBLOCK_ID"=>1, "ACTIVE"=>"Y", "CODE" => $_GET["city"]);
    $res = CIBlockElement::GetList(Array("sort"=>"asc"), $arFilter, false, Array("nPageSize"=>50), $arSelect);
    while($ob = $res->GetNextElement())
    {
        $arFields = $ob->GetFields();
        $_SESSION["CITY_SELECT"]["NAME"] = $arFields["NAME"];
        $_COOKIE['CITY_SELECT']['NAME'] = $arFields['NAME'];
//        $_SESSION["CITY_SELECT"]["PHONE"] = $arFields["PROPERTY_PHONE_VALUE"];
    }
}T', 'JM', 'JO', 'JP', 'KE',
