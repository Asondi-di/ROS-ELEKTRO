	...
	if($arParams["SHOW_GENERAL_STORE_INFORMATION"] != "Y"){
		foreach($arResult["STORES"] as $pid => $arProperty){
			...
			if(isset($arStore["UF_METRO"]))
				$arResult["STORES"][$pid]["METRO"] = unserialize($arStore["UF_METRO"], ['allowed_classes' => false]);
			if($arResult["STORES"][$pid]["PHONE"]){

			}
			if($arResult["STORES"][$pid]["PHONE"]){
				$arResult["STORES"][$pid]["PHONE"] = implode(', ', array_merge(explode(",", $arResult["STORES"][$pid]["PHONE"]), (array)unserialize($arStore['UF_PHONES'], ['allowed_classes' => false]) ?: array()));
			}elseif(isset($arStore["UF_PHONES"])){
				$arResult["STORES"][$pid]["PHONE"] = implode(', ', (array)unserialize($arStore['UF_PHONES'], ['allowed_classes' => false]));
			}
		...
		while ($prop = $rsProps->GetNext()){
			...
			if(isset($prop["UF_METRO"]))
				$arResult["STORES"][$prop['ID']]["METRO"] = unserialize($prop["UF_METRO"], ['allowed_classes' => false]);
		}
