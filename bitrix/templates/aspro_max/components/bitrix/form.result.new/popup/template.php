...
<div class="form <?=$arResult["arForm"]["SID"]?>  ">
	...
	<div class="form_head">
		<?if($arResult["isFormTitle"] == "Y"):?>
			<?
			//luckru
			//$_REQUEST['RESULT_ID'] = 13;
			//$arResult["FORM_NOTE"] = "DS";
			$strFormTitle = $arResult["FORM_TITLE"];
			if($_REQUEST['RESULT_ID'] > 0) 
			{
				$strFormTitle = "Спасибо за вашу заявку!";
			}
			?>
		...
		<?if($arResult["isFormDescription"] == "Y"):?>
			<? if($_REQUEST['RESULT_ID'] > 0): ?>
				<br>
				<p class="form_send_success_text">
					В ближайшее время мы свяжемся с вами<br>
					по телефону, чтобы уточнить детали заявки.<br>
                    Вы можете <a target="_blank" href="/contacts/">позвонить</a> нам или дождаться<br>
                    звонка менеджера.
				</p>
		    <? else:?>
		    	<br><?= $arResult["FORM_DESCRIPTION"] ?>
			<? endif; ?>	 
		<?endif;?>
	</div>
	<?if(strlen($arResult["FORM_NOTE"])){?>
		<div class="form_result <?=($arResult["isFormErrors"] == "Y" ? 'error' : 'success')?>">
			...
			<?else:?>
				<?// echo CMax::showIconSvg(' colored', SITE_TEMPLATE_PATH.'/images/svg/success.svg')?>
